...
class SlowClipDataset(Dataset):
    ...
    def get_segments(self):
        ...
        for video_id in self.video_ids:
            ...
            # this ensure at least segment included
    ...
    def get_label(self, segment):
        ...
        0 is normal version (range is 1-1.2x speed)
        1 is slow down (0.7-0.9x speed)

        Note: counter-intuitive:  videos are already slowed down by 0.5x.
        ie, so we need to speed up to make it normal

        This will sample either a normal or slowed label, and sample a skip factor f,
        which will skip a label within a range by probability p, producing the
        desired speed of the video.  p = 1 - 1/f
        ...
        skip_factor = random.uniform(f_range[0], f_range[1])
...
def save_frames(frames, video_id, label):
    ...
    out_path = '/vision2/u/enguyen/results/slow_frames_test/'
